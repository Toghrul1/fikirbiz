...
function App() {
  ...
  return (
    <div className="min-h-screen bg-brand-ivory font-sans text-brand-navy">
      ...
      />
      <header className="relative z-20 flex items-center justify-end gap-2 sm:gap-3 p-3 sm:p-4">
        <LanguageSwitcher value={language} onChange={setLanguage} />
        <button
          onClick={handleLogout}
          className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-2 bg-white border border-brand-gray rounded-xl text-xs sm:text-sm text-brand-navy hover:border-red-400 hover:text-red-500 transition-colors"
        >
