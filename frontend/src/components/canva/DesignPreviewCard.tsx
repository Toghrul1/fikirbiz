...
export const DesignPreviewCard: React.FC<DesignPreviewCardProps> = ({ design, compact = false }) => {
  ...
  return (
    <div className="rounded-2xl border border-brand-gray/20 bg-white shadow-sm overflow-hidden">
      {/* Thumbnail */}
      <div className="relative aspect-video bg-gradient-to-br from-brand-gold/20 to-brand-navy/10 flex items-center justify-center">
        {design.thumbnailUrl ? (
          <img
          ...
          <div className="flex flex-col items-center gap-2 text-brand-gray/50">
        ...
        {/* Content Type Badge */}
        <div className="absolute top-2 left-2 px-2 py-1 rounded-md bg-brand-navy/80 text-white text-xs font-medium">
          {design.contentType}
        </div>
      </div>
      ...
      {/* Content */}
      <div className="p-4">
        <h3 className="font-medium text-brand-navy truncate">
          {design.title || 'Canva Dizaynı'}
        </h3>
        {design.createdAt && (
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-brand-gold text-brand-navy text-sm font-medium hover:bg-brand-gold/90 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg border border-brand-gray/30 text-brand-navy text-sm font-medium hover:bg-brand-ivory transition-colors"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
