...
const toastBgColors: Record<ToastType['type'], string> = {
...
const ToastItem: React.FC<ToastItemProps> = ({ toast, onRemove }) => {
  ...
  return (
    <div
      className={`flex items-start gap-3 p-4 rounded-xl border shadow-lg ${toastBgColors[toast.type]} animate-slide-in`}
    >
      <div className="shrink-0 mt-0.5">{toastIcons[toast.type]}</div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-brand-navy">{toast.title}</p>
        {toast.message && (
          <p className="mt-1 text-sm text-brand-gray">{toast.message}</p>
        )}
      </div>
      <button
        onClick={() => onRemove(toast.id)}
        className="shrink-0 text-brand-gray/50 hover:text-brand-navy transition-colors"
      >
